...
/**
 * Renders assembled document markdown as a real PDF — parses it into an
 * mdast tree (the same remark/remark-gfm stack used for the on-screen
 * preview) and walks that tree into `@react-pdf/renderer` primitives, so the
 * downloaded file has selectable, searchable text rather than a rasterized
 * snapshot. Shared by every document type's preview.
 */
export function MarkdownPdfDocument({ markdown, title }: MarkdownPdfDocumentProps) {
